...
import { getUniqueSuppliers, todayIso } from '@/utils/entryHelpers';
...
export default function AddPurchaseScreen() {
  ...
  const supplierSuggestions = useMemo(() => getUniqueSuppliers(purchases), [purchases]);
  ...
  useEffect(() => {
    ...
    if (params.duplicateFrom) {
      const entry = purchases.find(p => p.id === params.duplicateFrom);
      if (entry) {
        setName(entry.name);
        setTotalAmount(entry.amount.toString());
        if (entry.qty) { setQty(entry.qty.toString()); setAmountMode('unit'); }
    ...
    if (templateData) {
      const p = templateData.payload;
      setName(p.name ?? '');
      setCategory(p.category ?? 'Wood');
      setSupplier(p.supplier ?? '');
      setNote(p.note ?? '');
      if (p.qty) { setQty(p.qty.toString()); setAmountMode('unit'); }
      if (p.unitPrice) setUnitPrice(p.unitPrice.toString());
      if (templateData.defaultAmount) setTotalAmount(templateData.defaultAmount.toString());
    }
  }, [editId, params.duplicateFrom, templateData, purchases, today]);
  ...
  async function handleSaveAsTemplate() {
    if (!name.trim()) { showToast(t('toasts.itemNameRequired'), 'error'); return; }
    ...
    await addTemplate({
      ...
      defaultAmount: computedTotal || undefined,
    });
  ...
  async function handleSave() {
    ...
    if (amt <= 0) { showToast(t('toasts.validAmount'), 'error'); return; }
    ...
    try {
      if (editId) {
        ...
        showToast(t('toasts.purchaseUpdated'));
      } else {
        await addPurchase(data);
        showToast(t('toasts.purchaseAdded'));
      }
      router.back();
    } catch {
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.background }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[styles.header, { paddingTop: topPad + 8, backgroundColor: colors.background, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>
          {editId ? t('forms.editPurchase') : t('forms.addPurchase')}
        </Text>
        <TouchableOpacity
          onPress={handleSave}
          onLongPress={isNewEntry ? handleSaveAsTemplate : undefined}
          style={[styles.saveBtn, { backgroundColor: colors.purchases }]}
          activeOpacity={0.8}
        >
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
            ...
            {(['total', 'unit'] as AmountMode[]).map(m => (
              ...
              >
                <Text style={[styles.modeBtnText, { color: amountMode === m ? '#fff' : colors.purchases }]}>
                  {m === 'total' ? t('forms.totalAmount') : t('forms.qtyPrice')}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        <View style={[styles.form, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={styles.fieldRow}>
            ...
            <TextInput
              ...
              placeholder={t('forms.supplierOptional')}
              placeholderTextColor={colors.mutedForeground}
              returnKeyType="next"
            />
          </View>
          <SuggestionChips suggestions={supplierSuggestions} onSelect={setSupplier} />
          <View style={[styles.divider, { backgroundColor: colors.border }]} />
          <View style={styles.fieldRow}>
            <View style={[styles.fieldIcon, { backgroundColor: colors.secondary }]}>
        ...
        <View style={[styles.form, { backgroundColor: colors.card, borderColor: colors.border, gap: 10, padding: 14 }]}>
          <Text style={[styles.catTitle, { color: colors.foreground }]}>{t('forms.category')}</Text>
          <View style={styles.catGrid}>
            {PURCHASE_CATEGORIES.map(cat => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Text style={[styles.catChipText, { color: category === cat ? '#fff' : colors.mutedForeground }]}>{categoryLabel(cat)}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <View style={[styles.form, { backgroundColor: colors.card, borderColor: colors.border, gap: 12, padding: 16 }]}>
          <Text style={[styles.catTitle, { color: colors.foreground }]}>{t('forms.receiptPhoto')}</Text>
          {photo ? (
