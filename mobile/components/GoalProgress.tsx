...
export function GoalProgress({ current, goal }: GoalProgressProps) {
  ...
  const formatted = (n: number) => formatNumber(n, language, { minimumFractionDigits: 0, maximumFractionDigits: 0 });
  const barColor = pct >= 100 ? colors.positive : pct >= 60 ? colors.primary : colors.warning;

  return (
    <View style={[styles.container, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={styles.header}>
        <Text style={[styles.label, { color: colors.mutedForeground }]}>{t('home.monthlyGoal')}</Text>
