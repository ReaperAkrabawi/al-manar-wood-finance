...
export default function StatsScreen() {
  ...
  const summaryCards = [
    { label: 'Total Income', value: allTimeIncome, color: colors.income, bg: colors.incomeLight },
    { label: 'Total Purchases', value: allTimePurchases, color: colors.purchases, bg: colors.purchasesLight },
    { label: 'Total Wages', value: allTimeWages, color: colors.wages, bg: colors.wagesLight },
    { label: 'Total Withdrawals', value: allTimeWithdrawals, color: colors.withdrawals, bg: colors.withdrawalsLight },
    { label: 'Net Balance', value: allTimeNet, color: allTimeNet >= 0 ? colors.positive : colors.negative, bg: allTimeNet >= 0 ? colors.incomeLight : colors.destructive + '18' },
  ];
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={[styles.header, { paddingTop: topPad + 8, backgroundColor: colors.background, borderBottomColor: colors.border }]}>
        ...
        <View style={[styles.tabBar, { backgroundColor: colors.secondary }]}>
          {(['overview', 'history'] as StatsTab[]).map(t => (
            <TouchableOpacity
              key={t}
              onPress={() => setActiveTab(t)}
              style={[styles.tabBtn, activeTab === t && { backgroundColor: colors.card, shadowOpacity: 0.08, shadowRadius: 4, elevation: 2 }]}
            >
              <Text style={[styles.tabLabel, { color: activeTab === t ? colors.primary : colors.mutedForeground }]}>
                {t === 'overview' ? 'Overview' : 'Purchases History'}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
      ...
      {activeTab === 'overview' ? (
        ...
        >
          ...
          <View style={[styles.formulaCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={[styles.formulaTitle, { color: colors.mutedForeground }]}>Net Balance Formula</Text>
            <View style={styles.formulaRow}>
              <FormulaItem label="Income" value={allTimeIncome} color={colors.income} sign="+" fmt={fmt} />
              <Text style={[styles.formulaOp, { color: colors.mutedForeground }]}>−</Text>
              <FormulaItem label="Purchases" value={allTimePurchases} color={colors.purchases} fmt={fmt} />
              <Text style={[styles.formulaOp, { color: colors.mutedForeground }]}>−</Text>
              <FormulaItem label="Wages" value={allTimeWages} color={colors.wages} fmt={fmt} />
              <Text style={[styles.formulaOp, { color: colors.mutedForeground }]}>−</Text>
              <FormulaItem label="Withdrawals" value={allTimeWithdrawals} color={colors.withdrawals} fmt={fmt} />
            </View>
          </View>
            ...
            <View style={styles.sectionHeader}>
              ...
              <View style={styles.legend}>
                <View style={styles.legendItem}>
                  ...
                  <Text style={[styles.legendLabel, { color: colors.mutedForeground }]}>Income</Text>
                </View>
                <View style={styles.legendItem}>
          ...
          {catBarData.length > 0 && (
            <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Purchases by Category</Text>
          ...
          {workerWages.length > 0 && (
            <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Wages by Worker</Text>
        ...
        <View style={{ flex: 1 }}>
          <View style={[styles.historyHeader, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
            <Text style={[styles.histCount, { color: colors.mutedForeground }]}>{sortedPurchases.length} purchases</Text>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.empty}>
                ...
                <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No purchases yet</Text>
              </View>
            }
            renderItem={({ item: p }) => (
              <EntryCard
                ...
                onDelete={() => { deletePurchase(p.id); showToast('Purchase deleted'); }}
